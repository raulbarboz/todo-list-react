  ...
  render(){
    return(
      <li className={"list-group-item " + (this.state.tempoRestante ? '' : 'list-group-item-danger')}>
        ...
        {this.state.toggleEdit ? <input className="inputEdit" onFocus={this.handleFocus} value={this.state.todo} onChange={this.handleChange} type="text"/> : <h4>{this.props.todoItem.todo}</h4>}
